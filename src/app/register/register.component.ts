...
export class RegisterComponent implements OnInit {
  ...
  register() {
    ...
    if (val.username && val.password && val.email && val.password_repeat) {
      this.authService.register(val.username, val.password, val.email, val.first_name, val.last_name).then(data=>{
        if(data.status === 406){
          this.errorLabelComponent.genereateError("User already exists!", "danger", "The entered e-mail adress is not valid!");
